...
def find_latest_post_image(res_data):
    """Get the image URL directly from the latest Instagram post."""
    try:
        items = res_data["result"]["data"]["items"]

        if not items:
            return None

        # First item is the latest post returned by the Instagram API
        latest_post = items[0]

        # Get the actual post image
        image_versions = latest_post.get("image_versions", {})
        image_items = image_versions.get("items", [])

        if image_items:
            return image_items[0].get("url")
        ...
        print(f"⚠️ Could not find latest post image: {e}")
        return None
...
try:
    ...
    if response.status_code == 200:
        ...
        image_url = find_latest_post_image(res_data)

        if image_url:
            print(f"📸 Found latest post image URL: {image_url[:80]}...")
            ...
            if img_res.status_code == 200:
                ...
                print(f"⚠️ Failed to download image. Status Code: {img_res.status_code}")

        else:
            print("⚠️ No valid image URL found in the latest Instagram post.")
...
for veg in parsed_veggies:
    ...
    change_pct = round(
        (change / yesterday_price) * 100, 1
    ) if yesterday_price > 0 else 0
...
print(
    ...
    f"{len(final_vegetables)} vegetables into vegetables_data.json!"
)
